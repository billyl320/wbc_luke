...
def bin_edges (fname, bname):#, thresh=2 ):
    ...
    shape = (clist[1]+0)
    sm.imsave(bname+fname[51:-3]+'png', shape+0.0)
    shape = (clist[1]+0)
    ############
    #fill holes
    ############
...
def otsu(fname, bname):
    ...
    ###########################
    #make black pixels yellow to help with different data sets
    ###########################
    adata2 = adata+0.0
    v,h,d = adata.shape
    for i in range(v):
        for j in range(h):
            if adata2[i,j,0]==adata2[i,j,1]==adata2[i,j,2]==0.00:
                adata2[i,j,0]=255.0
                adata2[i,j,1]=255.0
                adata2[i,j,2]=0.0
    ###########################
...
def GetAllImages_Colors( dirs, og):
    ...
    for j in mgnames:
        ...
        d = sm.imread(og[0]+j[12:-4]+'.bmp', flatten=False)
...
def GetAllImages_Texture( dirs, og):
    ...
    for j in mgnames:
        ...
        b = (b>125.0) +0.0
        #read in original image
        d = sm.imread(og[0]+j[12:-4]+'.bmp', flatten=True)
